<template>
  <div class="coin-wrapper">
    ...
    <div class="coin-shadow" :class="coinClass"></div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    const coinClass = computed(() => {
      if (props.state.phase === 'flipping') return 'flipping'
      if (props.state.phase === 'resolved' && props.state.outcome) return props.state.outcome.side
      return 'idle'
    })
    return { coinClass }
  },
...
.coin-shadow.flipping { width: 80px; opacity: 0.5; }
...
@media (max-width: 640px) {
  .coin-wrapper { width: 140px; height: 140px; }
