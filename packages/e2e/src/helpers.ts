...
/**
 * Send `amountBtc` to a regtest address via the arkade-regtest CLI, mining 1
 * block so it confirms (the in-house replacement for the chopsticks HTTP
 * `/faucet`). Shared by the per-suite faucet helpers.
 */
export function cliFaucet(address: string, amountBtc: number): void {
  regtestCli(['faucet', address, String(amountBtc), '--confirm'])
}
